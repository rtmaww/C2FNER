...
def get_protos_and_variance_from_model(model, dataloader, target_layer=-1, cluster_num=1000):
    ...
    for i in range(cluster_num):
        embeds = torch.stack(cluster_embeds[i])
        cluster_mean.append(torch.mean(embeds, dim=0))
        cluster_cov.append(torch_cov(embeds))

    cluster_means = torch.stack(cluster_mean)
    cluster_covs = torch.stack(cluster_cov)
    print(f"models/{save_model}")
    torch.save(cluster_means, f'models/{save_model}/cluster_means.pt')
    torch.save(cluster_covs, f'models/{save_model}/cluster_covs.pt')
...
if __name__ == '__main__':
    ...
    get_protos_and_variance_from_model(model, train_dataloader, cluster_num=cluster_num)
